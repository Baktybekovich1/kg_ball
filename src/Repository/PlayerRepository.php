...
SQL;
    ...
    public function getBestPlayersInMonth(string $startDate , string $endDate): array
    {
        $sql = <<<SQL
SELECT p.id as playerid,
       ...
       COALESCE(goals.goals_count, 0) + COALESCE(assists.assists_count, 0) AS total_points
FROM player p
         LEFT JOIN (SELECT g.player_id,
                           COUNT(g.id) AS goals_count
                    FROM goal g
                             INNER JOIN game ga ON g.game_id = ga.id
                             INNER JOIN tourney t ON ga.tourney_id = t.id
                    WHERE t.date between :startDate and :endDate
                    GROUP BY g.player_id) goals ON p.id = goals.player_id
         LEFT JOIN (SELECT a.player_id,
                           COUNT(a.id) AS assists_count
                    FROM assist a
                             INNER JOIN goal g ON a.goal_id = g.id
                             INNER JOIN game ga ON g.game_id = ga.id
                            INNER JOIN tourney t ON ga.tourney_id = t.id
                    WHERE t.date between '2025-04-01' and '2025-05-01'
                    GROUP BY a.player_id) assists ON p.id = assists.player_id
...
SQL;
        ...
        return $this->conn->fetchAllAssociative($sql, ['startDate' => $startDate, 'endDate' => $endDate]);
    }
